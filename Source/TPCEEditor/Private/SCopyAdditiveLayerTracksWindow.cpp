...
FReply SCopyAdditiveLayerTracksWindow::OnApply()
{
	...
	if (!SourceAnimSequence.IsValid() || !SourceAnimSequence->DoesContainTransformCurves())
	{
		...
		const auto Title =  LOCTEXT("CopyAdditiveLayerTracksWindow_NoSourceTracksTitle", "No source additive layer tracks");
		const bool bUserInputResult = FMessageDialog::Open(EAppMsgType::YesNo, MessageFormat, &Title) == EAppReturnType::Yes;
		if (!bUserInputResult)
		{
